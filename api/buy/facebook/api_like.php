    ...
    case 'getlist':
        if (isset($_POST['token']) && isset($_POST['limit'])) {
            ...
            if ($tko == '1') {
                ...
                $quan = [];
                if ($sort == 'desc') {
                    $result1 = mysqli_query($db, "SELECT * FROM `dichvu` WHERE `dv` = 'Like' AND (`trangthai` = '1') AND `sve` = 'Server Like Tay' ORDER BY id DESC LIMIT $limit");
                } elseif ($sort == 'asc') {
                    $result1 = mysqli_query($db, "SELECT * FROM `dichvu` WHERE `dv` = 'Like' AND (`trangthai` = '1') AND `sve` = 'Server Like Tay' ORDER BY id ASC LIMIT $limit");
                } else {
                    $result1 = mysqli_query($db, "SELECT * FROM `dichvu` WHERE `dv` = 'Like' AND (`trangthai` = '1') AND `sve` = 'Server Like Tay' ORDER BY id DESC LIMIT $limit");
                }
    ...
    case 'updatedata':
        if (isset($_POST['token']) && isset($_POST['id_order'])) {
            ...
            if ($tko == '1') {
                $row = mysqli_fetch_assoc($uu);
                $login = $row['username'];
                $check = mysqli_query($db, "SELECT * FROM `dichvu` WHERE `dv`='Like' AND `id` = '$id_order'");
                $c = mysqli_num_rows($check);
                if ($c == '1') {
                    $array["status"] = 'success';
                    mysqli_query($db, "UPDATE `dichvu` SET `done` = '$done', `trangthai` = '$status' WHERE `id` = '$id_order' AND `dv` = 'Like'");
