...
if (isset($_POST['token']) && isset($_POST['id']) && isset($_POST['sl']) && isset($_POST['sv'])) {
    ...
    if ($tko == '1') {
        ...
        if (empty($id)) {
            ...
            if ($sv == 1) {
                $order = $api->order(array('service' => 99, 'link' => '' . $id . '', 'quantity' => $sl));
            } elseif ($sv == 2) {
                $order = $api->order(array('service' => 1487, 'link' => '' . $id . '', 'quantity' => $sl));
            }
            if (isset($order)) {
                $nd1 = 'Tăng View Instagram ID:';
                $bd = $tongtien;
                $gt = '-';
                $idgd = '' . $id . ' (' . $sl . ')';
                $goc = $row['vnd'];
                ...
                mysqli_query($db, "INSERT INTO `dv_other` SET `dv` = 'ins_view',`sl` = '$sl', `trangthai` = '2', `user`='$login',`profile`='$id',`time` = '$time', `sttdone` = '$sl', `sotien` = '$tongtien', `done` = '$sl', `nse` = '$nse'");
