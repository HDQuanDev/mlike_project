...
if (time() > 0) {
    ...
    if ($output == '1') {
        ...
        $url2 = 'https://traodoisub.com/mua/like/';
        // Khởi tạo CURL
        $ch2 = curl_init($url2);
        // Thiết lập có return
        ...
        foreach ($lol->find('tr') as $article) {
            ...
            $datang = $article->find('td', 2)->innertext;
            $check = mysqli_query($db, "SELECT * FROM `dichvu` WHERE `profile` = '$Idlike' AND `sttdone` = '0' AND `nse` = '3' ORDER BY id DESC LIMIT 20");
            $check = mysqli_num_rows($check);
            if ($check != 0) {
                $u = mysqli_query($db, "SELECT * FROM `dichvu` WHERE `profile`='$Idlike' AND `nse` = '3' AND `sttdone` = '0' LIMIT 1");
                $u = mysqli_fetch_assoc($u);
                $goc = $u['sl'];
                mysqli_query($db, "UPDATE `dichvu` SET `done` = '$datang' WHERE `profile` = '$Idlike' AND `nse` = '3' AND `sttdone` = '0'");
                echo 'ID: <a href="//facebook.com/' . $Idlike . '">' . $Idlike . '</a> -> DONE: ' . $datang . ' / ' . $goc . '<br>';
                if ($datang >= '1' && $datang < $goc) {
                    mysqli_query($db, "UPDATE `dichvu` SET `trangthai` = '3' WHERE `profile` = '$Idlike' AND `nse` = '3' AND `sttdone` = '0'");
                } elseif ($datang > $goc) {
                    mysqli_query($db, "UPDATE `dichvu` SET `trangthai` = '2', `sttdone` = '1' WHERE `profile` = '$Idlike' AND `nse` = '3'");
                }
            }
        }
    }
