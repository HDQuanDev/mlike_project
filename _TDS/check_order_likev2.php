...
$result1 = mysqli_query($db,"SELECT * FROM `dv_other` WHERE `dv` = 'fblike_v2' AND `sttdone` = '0' ORDER BY id DESC LIMIT 10");
...
if($quan->data){
  ...
  mysqli_query($db,"UPDATE `dv_other` SET `done` = '$success' WHERE `iddon` = '$idgd'");
    echo 'id: '.$ro['id'].' -> '.$success.'<br>';
    $goc = $ro['sl'];
    if($success >= 1 && $success < $goc){
        mysqli_query($db,"UPDATE `dv_other` SET `trangthai` = '3' WHERE `iddon` = '$idgd'");
    }elseif($success >= $goc){
        mysqli_query($db,"UPDATE `dv_other` SET `trangthai` = '2', `sttdone` = '1' WHERE `iddon` = '$idgd'");
    }
}
}