...
window.location="' . $url . '";
                    ...
                    $tongtien = $sl * $gia;
                    if (empty($id)) {
                        echo "<script>swal('OOPS!','Vui lòng nhập Link hoặc ID shop cần tăng','warning');</script>";
                    } elseif (empty($sl)) {
                        ...
                        $buff = shopee("$id", "$sl", "follow");
                        $buff = json_decode($buff);
                        if ($buff->status == 200) {
                            $nd1 = 'Tăng Follow Shopee ID:';
                            $bd = $tongtien;
                            $gt = '-';
                            $idgd = '' . $id . ' (' . $sl . ')';
                            $goc = $row['vnd'];
                            ...
                            mysqli_query($db, "INSERT INTO `dv_other` SET `dv` = 'shopee_follow',`sl` = '$sl', `trangthai` = '1', `user`='$login',`profile`='$id',`time` = '$time', `sttdone` = '0', `sotien` = '$tongtien', `done` = '0'");
                            mysqli_query($db, "UPDATE `member` SET `vnd` = `vnd`-'$tongtien', `sd` = `sd`+'$tongtien' WHERE `username` = '$login' AND `site` = '$site'");
                            echo "<script>swal('Hệ Thống!','Tăng Follow Thành Công! Cảm ơn bạn!!','success');</script>";
    ...
    window.location="' . $url . '";
        ...
        <div class="card border-danger border-bottom border-3 border-0">
            <div class="card-header">
                <h5 class="card-title" data-anchor="data-anchor">Tăng Follow Shopee</h5>
            </div>
            <div class="card-body">
                <p class="mb-0">
                <form action="" id="form_id" class="user" method="POST" accept-charset="utf-8">
                    ...
                    <label><strong> CÁCH LẤY LINK NHƯ HÌNH BÊN DƯỚI</strong></label>
                    <hr><img class="img-fluid rounded" src="https://i.imgur.com/xcHxDLB.png" alt="" />
                    <hr class="md-1">

                    <div class="mb-3">
                        <label><strong>Nhập ID và Link shop:</strong></label>
                        <div class="input-group mb-3">
                            <input type="text" oninput="calc()" name="id" class="form-control" aria-label="Recipient's username" aria-describedby="basic-addon2" placeholder="Nhập Link rồi bấm get ID!" required="" id="idbuff_like">
                            <div class="input-group-append">
                                <button class="btn btn-outline-warning" id="get" type="button">GET ID</button>
                            </div>
                        </div>
                        <label>(*) Nếu nhập <code>link</code> vui lòng bấm <code>"GET ID"</code></label>
                    </div>
                    ...
                    <div class="alert alert-warning" role="alert">
                        ...
                        - Không thể hủy đơn khi đã thanh toán ( Không hoàn tiền ) <br>
                        - Nếu gặp lỗi hãy nhắn tin hỗ trợ phía bên phải góc màn hình để được hỗ trợ tốt nhất !<br>
                    </div>

                    <div class="mb-3">
                        <label> <strong> Số Lượng Follow Muốn Mua: </strong> </label>
                        <input type="number" id="sl" oninput="calc()" class="form-control" placeholder="Nhập số..." name="sl" value="" required="">
                    </div>

                    <div class="alert alert-success" role="alert">
                        <center><strong>Giá: <?= $gia; ?> ₫ / 1 Follow<br>Cách Tính Giá: Giá x Số Follow
