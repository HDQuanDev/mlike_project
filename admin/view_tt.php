...
$title = "Quản Lý View Tay Tiktok";
require_once('../_System/head.php');
?>
<div class="card border-danger border-bottom border-3 border-0">
    <div class="card-header">
        <h5 class="card-title">Danh Sách Đơn Hàng View TIKTOK Thủ Công </h5>
    </div>
    <div class="card-body">
        <div class="table-responsive scrollbar">
            ...
            if ($_GET['action']) {
                $id = $_GET['id'];
                $tt = $_GET['action'];
                mysqli_query($db, "UPDATE `dv_other` SET `trangthai` = '$tt', `auto` = '1' WHERE `id` = '$id'");
...
?>
        ...
        if ($_GET['status']) {
            $dv = $_GET['status'];
            $result1 = mysqli_query($db, "SELECT * FROM `dv_other` WHERE `trangthai` = '$dv' AND `dv` = 'tiktok_view' AND `nse` = 'Server View 1' ORDER BY id DESC LIMIT 3000");
        } else {
            $result1 = mysqli_query($db, "SELECT * FROM `dv_other` WHERE `dv` = 'tiktok_view' AND `nse` = 'Server View 1' ORDER BY id DESC LIMIT 1500");
        }
if ($result1) {
    while ($ro = mysqli_fetch_assoc($result1)) {
        ...
        ?>
                    ...
                    if ($tt == 3) {
                        ...
                        echo '<td class="done">Không thể chỉnh sửa<br> <strong>View Gốc: ' . $ro['iddon'] . '</strong></td>';
                    }
        ?>
                                <?php
        if ($tt == 3) {
