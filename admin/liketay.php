...
$title = "Quản Lý Like Tay";
require_once('../_System/head.php');
?>
<div class="card border-danger border-bottom border-3 border-0">
    <div class="card-header">
        <h5 class="card-title">Danh Sách Đơn Hàng Thủ Công </h5>
    </div>
    <div class="card-body">
...
if (isset($_POST['gc'])) {
    $id = $_POST['id'];
    $goc = $_POST['goc'];
    mysqli_query($db, "UPDATE `dichvu` SET `idgd` = '$goc' WHERE `id` = '$id'");
...
?>
            <table class="table table-striped table-bordered" id="example">
                <thead class="bg-200 text-900">
                    <tr>
                        <th class="sort" data-sort="id"><b>#</b><b>#</b></th>
                        ...
                        <th class="sort" data-sort="sv">Server LIKE</th>
        ...
        if ($_GET['status']) {
            $dv = $_GET['status'];
            $result1 = mysqli_query($db, "SELECT * FROM `dichvu` WHERE `trangthai` = '$dv' AND `bh` = '1' AND (`nse` = '4' OR `nse` = 'Server Share 3' OR `nse` = 'Server Share 4' OR `nse` = 'Server Share 5' OR `nse` = 'Server Follow 3') ORDER BY id DESC LIMIT 100");
        } else {
            $result1 = mysqli_query($db, "SELECT * FROM `dichvu` WHERE (`bh` = '1' OR `bh` = '3') AND (`nse` = '4' OR `nse` = 'Server Share 3' OR `nse` = 'Server Share 4' OR `nse` = 'Server Share 5' OR `nse` = 'Server Follow 1') ORDER BY id DESC LIMIT 100");
        }
if ($result1) {
    ...
    while ($ro = mysqli_fetch_assoc($result1)) {
        ...
        if ($tt == 1 || $tt == 3) {
            ?>
                                    <td class="cn"><a href="?action=3&id=<?= $ro['id']; ?>">Đang Chạy</a> | <a href="?action=4&id=<?= $ro['id']; ?>">Bị Hủy</a> <?php if ($ro['nse'] == 'Server Share 4' || $ro['nse'] == 'Server Share 5') { ?>| <a href="?action=7&id=<?= $ro['id']; ?>">Hủy Hoàn</a> <?php } ?>| <a href="?action=2&id=<?= $ro['id']; ?>">Hoàn Thành</a></td>
                                <?php
        } else {
            echo '<td class="cn"></td>';
        }
