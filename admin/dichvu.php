...
if($_GET['status']){
	$dv = $_GET['status'];
				$result1 = mysqli_query($db,"SELECT * FROM `dichvu` WHERE `trangthai` = '$dv' AND `bh` = '1' AND `nse` < '3' ORDER BY id DESC LIMIT 500");
}else{
	$result1 = mysqli_query($db,"SELECT * FROM `dichvu` WHERE `bh` = '1' AND `nse` < '3' ORDER BY id DESC LIMIT 500");
	}
				if($result1)
				{
...
<td class="profile"><a href="https://facebook.com/<?php echo $ro['profile']; ?>" target="_blank"><?php echo $ro['profile']; ?></a> <?=$idgd;?></td>
